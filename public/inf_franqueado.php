<?php
  ...
  #$res=$cn->query("SELECT * FROM cliente WHERE status=1 ORDER BY id");
  #$idCliente=mysqli_fetch_assoc($res);
  #$idCliente=$_GET['id_venda'];//Adicionar o id que esta no loop
  #$dado2=mysqli_fetch_assoc($cn->query("SELECT * FROM docs_cliente ORDER BY id_cliente"));
  $idC=$_GET['id_cliente'];
  $res=$cn->query("SELECT * from docs_cliente WHERE id_cliente='$idC'");
  $st_venda=mysqli_fetch_assoc($cn->query("SELECT status FROM vendas WHERE id_cliente='$idC'"));
  $st_venda=$st_venda['status'];
  $st_P=mysqli_fetch_assoc($cn->query("SELECT st_pagamento FROM cliente WHERE id='$idC'"));
  $st_P=$st_P['st_pagamento'];
?>
<!DOCTYPE html>
<html lang="en-US" dir="ltr">
  ...
  <head>
    ...
    <!--    Document Title-->
    <!-- ===============================================-->
    <title>Emanuel | Documentaçõa</title>
  ...
  <body>
        ...
        <li class="nav-item dropdown"><a class="nav-link pe-0 ps-2" id="navbarDropdownUser" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
          ...
          <div class="dropdown-menu dropdown-caret dropdown-caret dropdown-menu-end py-0" aria-labelledby="navbarDropdownUser">
            <div class="bg-white dark__bg-1000 rounded-2 py-2">
              <a class="dropdown-item" href="./edit_user_gerente.php?pg=inf_franqueado.php&id_venda=<?php echo $_GET['id_venda']; ?>">Configurações</a>
          ...
          </div><h1 class="display-4 mb-3" style="text-align:center;">DOCUMENTAÇÃO</h1>
          ...
          <div class="card">
            <div class="card-header">
              <div class="row flex-between-end">
                <div class="col-auto align-self-center">
                  <h5 class="mb-0" data-anchor="data-anchor" id="example">Vendedor: <?php echo $_GET['nome_franqueado']; ?><a class="anchorjs-link " aria-label="Anchor" data-anchorjs-icon="#" href="#example" style="padding-left: 0.375em;"></a></h5>
            ...
            <?php #while($dado=$res->fetch_array()){ ?>
              <div class="card-body bg-light"> <!--INICIO LIST CLIENTES-->
                <div class="tab-content">
                  <div class="tab-pane preview-tab-pane active" role="tabpanel" aria-labelledby="tab-dom-8fe0360f-9b73-497a-aa81-9d3887241a07" id="dom-8fe0360f-9b73-497a-aa81-9d3887241a07">
                    <div class="accordion" id="accordionExample">
                      <div class="accordion-item">
                        <form action="../crud/estoque/sta_venda.php" method="POST">
                            <input name="id_cliente" value="<?php echo $_GET['id_cliente']; ?>" hidden/>
                            <input name="nome_franqueado" value="<?php echo $_GET['nome_franqueado']; ?>" hidden/>
                            <input name="nome_cliente" value="<?php echo $_GET['nome_cliente']; ?>" hidden/>
                            <h2 class="accordion-header" id="heading1">
                            <div class="btn-group" role="group" >
                              <a id="status" class="btn btn-success me-1 mb-1 dropdown-toggle" id="btnGroupVerticalDrop4" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Documentação incompleta</a>
                            <input hidden id="inf" value="-1" name="inf"/>
                            <div class="dropdown-menu" aria-labelledby="btnGroupVerticalDrop4">
                              <a class="dropdown-item" href="#" onclick="st(0)">Validando documentação</a>
                              <a class="dropdown-item" href="#" onclick="st(1)">Em processo</a>
                              <a class="dropdown-item" href="#" onclick="st(2)">Enviado</a>
                              <a class="dropdown-item" href="#" onclick="st(3)">Aprovado</a>
                              <a class="dropdown-item" href="#" onclick="st(4)">Compra feita</a>
                              <a class="dropdown-item" href="#" onclick="st(5)">Chegada do sistema</a>
                              <a class="dropdown-item" href="#" onclick="st(6)">Sistema enviado</a>
                              <a class="dropdown-item" href="#" onclick="st(7)">Finalizado</a>
                            </div>
                            <div class="form-check form-check-inline">
                              <input <?php if($st_P)echo 'checked' ?> class="form-check-input" id="inlineRadio1" type="radio" name="st_pagamento" value="1" />
                              <label class="form-check-label" for="inlineRadio1">Pago</label>
                            </div>
                            <div class="form-check form-check-inline">
                              <input <?php if(!$st_P)echo 'checked' ?> class="form-check-input" id="inlineRadio2" type="radio" name="st_pagamento" value="0" />
                              <label class="form-check-label" for="inlineRadio2">Não pago</label>
                            </div>
                            <button class="btn btn-falcon-success me-1 mb-1" type="submit">Atualizar</button>
                          </form>
                        </div>
                        <script>
                          st(<?php echo $st_venda; ?>);
                          function st(op){
                            let st=document.getElementById('status');
                            let inf=document.getElementById('inf');
                            if(op==-1){
                              st.innerHTML='Documentação Incompleta';
                            }else if(op==0){
                              st.innerHTML='Validando documentação';
                              inf.value=0;
                            }
                            else if(op==1){
                              st.innerHTML='Em processo';
                              inf.value=1;
                            }
                            else if(op==2){
                              st.innerHTML='Enviado';
                              inf.value=2;
                            }
                            else if(op==3){
                              st.innerHTML='Aprovado';
                              inf.value=3;
                            }
                            else if(op==4){
                              st.innerHTML='Compra feita';
                              inf.value=4;
                            }
                            else if(op==5){
                              st.innerHTML='Chegada do sistema';
                              inf.value=5;
                            }
                            else if(op==6){
                              st.innerHTML='Sistema enviado';
                              inf.value=6;
                            }
                            else if(op==7){
                              st.innerHTML='Finalizado';
                              inf.value=7;
                            }
                          }
                        </script>
                          <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse1" aria-expanded="false" aria-controls="collapse1">Cliente: <?php echo $_GET['nome_cliente'] ?></button>
                        </h2>
                          ...
                          <div class="accordion-body">
                            ...
                            <div class="table-responsive scrollbar">
                              <table class="table table-hover table-striped overflow-hidden">
                                ...
                                <tbody>
                                  <?php 
                                    #while($dado2=$res2->fetch_array()){ 
                                    
                                    #$res2=$cn->query("SELECT * FROM docs_cliente WHERE id_cliente='$idC'");
                                    #$idC=$idC+1;
                                    ...
                                    <tr class="align-middle">
                                      ...
                                      <td class="text-nowrap"><?php echo substr($dado['nome'],3) ?></td>
                                      <td class="text-end"><a href="../docsFiles/<?php echo $dado['nome']; ?>.pdf" target="iframe_download" class="btn btn-falcon-primary me-1 mb-1" type="button"><i class="fas fa-download"></i></a></td>
                                      <?php $st=$dado['status']; ?>
                                      <td class="text-end"><button onclick="send_docs(<?php echo $dado['id']; ?>,<?php echo $dado['status']; ?>,'<?php echo $dado['inf_docs_cliente']; ?>')" class="btn btn-falcon-<?php if($st==0)echo 'info';elseif($st==1)echo 'danger';elseif($st==2)echo 'success'; ?> me-1 mb-1" data-bs-toggle="modal" data-bs-target="#error-modal" type="button"><i class="fas fa-info"></i></button></td>
                                    </tr>
                                  <?php  
                                    /*$idDocs=$dado['id'];
                                    $st_Docs=$dado['status'];
                                    $msgDocs=$dado['inf_docs_cliente'];
                                    $inf_docs=[
                                        'docs'.$idDocs=>[
                                            'inf_docs' => $msgDocs,
                                            'st_docs' => $st_Docs
                                ...
                                <div class="modal-content position-relative">
                                  ...
                                  <div class="modal-body p-0">
                                    <div class="rounded-top-lg py-3 ps-4 pe-6 bg-light">
                                      <h4 class="mb-1" id="modalExampleDemoLabel">Informações da Documentação</h4>
                                    </div>
                                    <div class="p-4 pb-0">
                                      <form action="../crud/docs/inf_docs.php" method="post">
                                        <input value="<?php echo $_GET['id_cliente']; ?>" name="id_cliente" hidden/>
                                        <input value="<?php echo $_GET['nome_franqueado']; ?>" name="nome_franqueado" hidden/>
                                        <input value="<?php echo $_GET['nome_cliente']; ?>" name="nome_cliente" hidden/>
                                        <input id="id_docs_cliente" value="" name="id_docs_cliente" hidden/>
                                      <label class="col-form-label" for="message-text">Status da Documentação</label>
                                      <select id="st_docs" name="st_docs_cliente" class="form-select form-select-sm" aria-label=".form-select-sm example">
                                        <option id="op0" value="0">Em analise</option>
                                        <option id="op1" value="1">Indeferida</option>
                                        <option id="op2" value="2">Aprovada</option>
                                      </select>
                                        <div class="mb-3">
                                          <label class="col-form-label" for="message-text">Message:</label>
                                          <textarea id="msg_docs" name="msg" class="form-control" ></textarea>
                                        </div>
                                      
                                    </div>
                                  </div>
                                  <div class="modal-footer">
                                    <a class="btn btn-secondary" type="button" data-bs-dismiss="modal">Fechar</a>
                                    <input id="input_id" name="id_docs" value="" hidden />
          ...
          <a href="./vendas.php" class="btn btn-falcon-warning me-1 mb-1" type="button">Voltar</a>
                                      <script>
                                          function send_docs(id,st,msg){
                                            let Msg=document.getElementById('msg_docs');
                                            let stD=document.getElementById('st_docs');
                                            let op0=document.getElementById('op0');
                                            let op1=document.getElementById('op1');
                                            let op2=document.getElementById('op2');
                                            let idD=document.getElementById('id_docs_cliente');
                                            idD.value=id;
                                            Msg.value=msg;
                                            if(st==0){
                                              op0.value=0;
                                              op0.innerHTML='Em analise';
                                              op1.value=1;
                                              op1.innerHTML='Indeferida';
                                              op2.value=2;
                                              op2.innerHTML='Aprovada';
                                            }else if(st==1){
                                              op0.value=1;
                                              op0.innerHTML='Indeferida';
                                              op1.value=0;
                                              op1.innerHTML='Em analise';
                                              op2.value=2;
                                              op2.innerHTML='Aprovada';
                                            }else if(st==2){
                                              op0.value=2;
                                              op0.innerHTML='Aprovada';
                                              op1.value=0;
                                              op1.innerHTML='Em analise';
                                              op2.value=1;
                                              op2.innerHTML='Indeferida';
                                            }
                                          }
                                      </script>
          <footer class="footer">
    ...
    <div class="offcanvas offcanvas-end settings-panel border-0" id="settings-offcanvas" tabindex="-1" aria-labelledby="settings-offcanvas">
      ...
      <div class="offcanvas-body scrollbar-overlay px-card os-host os-theme-dark os-host-overflow os-host-overflow-y os-host-resize-disabled os-host-scrollbar-horizontal-hidden os-host-transition" id="themeController"><div class="os-resize-observer-host observed"><div class="os-resize-observer" style="left: 0px; right: auto;"></div></div><div class="os-size-auto-observer observed" style="height: calc(100% + 1px); float: left;"><div class="os-resize-observer"></div></div><div class="os-content-glue" style="margin: -16px -20px; width: 351px; height: 685px;"></div><div class="os-padding"><div class="os-viewport os-viewport-native-scrollbars-invisible" style="overflow-y: scroll;"><div class="os-content" style="padding: 16px 20px; height: 100%; width: 100%;">
        ...
        <div class="btn-group d-block w-100 btn-group-navbar-style">
          <div class="row gx-2">
            ...
            <div class="col-6">
              <input class="btn-check" id="navbar-style-inverted" type="radio" name="navbarStyle" value="inverted" data-theme-control="navbarStyle" checked="true">
      ...
      </div></div></div><div class="os-scrollbar os-scrollbar-horizontal os-scrollbar-unusable os-scrollbar-auto-hidden"><div class="os-scrollbar-track os-scrollbar-track-off"><div class="os-scrollbar-handle" style="width: 100%; transform: translate(0px, 0px);"></div></div></div><div class="os-scrollbar os-scrollbar-vertical os-scrollbar-auto-hidden"><div class="os-scrollbar-track os-scrollbar-track-off"><div class="os-scrollbar-handle" style="height: 50.6647%; transform: translate(0px, 0px);"></div></div></div><div class="os-scrollbar-corner"></div></div>
