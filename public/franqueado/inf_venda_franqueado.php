<?php 
    ...
    $sta_venda=mysqli_fetch_assoc($cn->query("SELECT status FROM vendas WHERE id_cliente='$idC'"));
    #$st=mysqli_fetch_assoc($cn->query("SELECT status FROM cliente WHERE id='$idC'"));
    $st=$sta_venda['status'];
...
<html lang="en-US" dir="ltr">
  ...
  <head>
    ...
    <!--    Document Title-->
    <!-- ===============================================-->
    <title>Emanuel | Documentação</title>
...
<div class="d-flex align-items-center"><img class="me-2" src="../assets/img/icons/spot-illustrations/falcon.png" alt="" width="40"><span class="font-sans-serif">Emanuel</span>
</div>
</a>
...
<div class="dropdown-menu dropdown-caret dropdown-caret dropdown-menu-end py-0" aria-labelledby="navbarDropdownUser">
      ...
      <div class="card mb-3">
        ...
        <div class="card-body position-relative">
          <div class="row">
            <div class="col-lg-8">
              <h3 style="text-align:center ;">Documentação</h3><div class="btn btn-falcon-success me-1 mb-1" type="button" id="decorrer_do_processo"><?php echo $inf; ?></div><br>
              <p class="mb-0"><?php echo $_GET['nome_cliente']; ?></p>
            </div>
          </div>
        </div>
      </div>
      <a class="btn btn-falcon-info me-1 mb-1" href="./vendas_franqueados.php"><i class="fas fa-arrow-left"></i></a>
      <button id="btAdd" onclick="changForm()" class="btn btn-falcon-success me-1 mb-1" type="button"><i class="fas fa-plus"></i></button>
      <div id="form2" class="card mb-3" hidden>
            <div class="card-header">
              <div class="row flex-between-end">
                <div class="col-auto align-self-center">
                  <h5 class="mb-0" data-anchor="data-anchor" id="sizing">Documentos<a class="anchorjs-link " aria-label="Anchor" data-anchorjs-icon="#" href="#sizing" style="padding-left: 0.375em;"></a></h5>
                </div>
              </div>
            </div>
            <div class="card-body bg-light">
              <form action="../../crud/docs/add_docs.php" method="POST" enctype="multipart/form-data">
              <div class="tab-content">
                <div class="tab-pane preview-tab-pane active" role="tabpanel" aria-labelledby="tab-dom-8cd50e7a-73b0-4421-894c-996358b82206" id="dom-8cd50e7a-73b0-4421-894c-996358b82206">
                <label class="form-label" for="exampleFormControlInput1">Nome</label>
                  <div class="mb-3">
                    <input name="nome" class="form-control" type="text" placeholder="" aria-label="default input example">
                  </div>
                  <div class="mb-3">
                    <input name="file" class="form-control" id="customFile" type="file" accept=".pdf"/>
                  </div>
                </div>
              ...
              </div><button class="btn btn-falcon-success me-1 mb-1" type="submit"><i class="fas fa-paper-plane"></i></button>
              <button onclick="changForm2()" class="btn btn-falcon-danger me-1 mb-1" type="button"><i class="fas fa-window-close"></i></button>
            </div>
          </div>
          </form>
      <div class="card mb-3">
        ...
        <div id="form1" class="card-body pt-0">
          <div class="tab-content">
            <div class="tab-pane preview-tab-pane active" role="tabpanel" aria-labelledby="tab-dom-1e5ba3af-8ba3-4f07-8e23-9c5a8a719928" id="dom-1e5ba3af-8ba3-4f07-8e23-9c5a8a719928">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="scrollinglongcontentLabel">Detetar Documentação?</h5>
        <button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body modal-dialog modal-dialog-scrollable mt-0">
        <p>O processo da documentação sera perdido, deseja realmente apaga-lo?</p>
      </div>
      <div class="modal-footer">
        <button class="btn btn-success" type="button" data-bs-dismiss="modal">NÃO</button>
        <form action="../../crud/docs/delDocsCliente.php" method="post">
          <input id="iddel" name="idDel" value="" hidden/>
          <input name="id_cliente" value="<?php echo $_GET['id_cliente']; ?>" hidden/>
          <input name="nome_cliente" value="<?php echo $_GET['nome_cliente']; ?>" hidden/>
          <button class="btn btn-danger" type="submit">SIM</button>
            ...
            <script>
              function status(op){
                ...
                if(op){
                  st.classList.remove('badge-soft-success');
                  st.classList.add('badge-soft-warning');
                }
              ...
              function changForm(){
                ...
                let send=document.getElementById('iddel');
                send.value=id;
                console.log(id);
              }
              function send_docs(st,Msg){
                console.log(st);
                let msg=document.getElementById('nota_docs_cliente');
                let op0=document.getElementById('status_docs_cliente');
                msg.innerHTML=Msg;
                if(st==0){
                  op0.innerHTML='Em andamento';
                }else if(st==1){
                  op0.innerHTML='Indeferido';
                }else{
                  op0.innerHTML='Aprovado';
                }
                //msg.innerHTML='TESTE'; 
                //msg.value='oioi';
              }
          ...
          <form>
            ...
            <label class="col-form-label" for="message-text">Status:</label>
              <select class="form-select" aria-label="Default select example" disabled>
                <option id="status_docs_cliente" selected="">xx</option>
              </select>
            </div>
            <div class="mb-3">
              <label class="col-form-label" for="message-text">Anotação:</label>
              <textarea id="nota_docs_cliente" class="form-control" disabled>xx</textarea>
            </div>
          </form>
        </div>
      </div>
      <div class="modal-footer">
        <button class="btn btn-falcon-warning me-1 mb-1" type="button" data-bs-dismiss="modal">Fechar</button>
      </div>
    </div>
  </div>
</div>
...
<div class="offcanvas offcanvas-end settings-panel border-0" id="settings-offcanvas" tabindex="-1" aria-labelledby="settings-offcanvas">
  ...
  </div></div></div><div class="os-scrollbar os-scrollbar-horizontal os-scrollbar-unusable os-scrollbar-auto-hidden"><div class="os-scrollbar-track os-scrollbar-track-off"><div class="os-scrollbar-handle" style="width: 100%; transform: translate(0px, 0px);"></div></div></div><div class="os-scrollbar os-scrollbar-vertical os-scrollbar-auto-hidden"><div class="os-scrollbar-track os-scrollbar-track-off"><div class="os-scrollbar-handle" style="height: 66.5436%; transform: translate(0px, 0px);"></div></div></div><div class="os-scrollbar-corner"></div></div>
